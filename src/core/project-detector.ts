...
export class ProjectDetector {
  ...
  async detect(botToken?: string): Promise<DetectedProject> {
    ...
    if (botToken) {
      try {
        ...
        githubProject = await this.detectGitHubProject(repoInfo.owner, repoInfo.name, botToken);
        if (githubProject) {
  ...
  private async detectRepository(): Promise<{
    ...
    try {
      ...
      const workingBranch = branches.all.includes('staging') ? 'staging' : undefined;
      ...
      throw new Error('Could not detect repository. Make sure you are in a git repository with GitHub remote.');
    }
  }
  ...
  private async detectBotUsername(token: string): Promise<string> {
    try {
      ...
      throw new Error(`Failed to authenticate with bot token: ${error?.message}`);
    }
  }
  ...
  private async detectGitHubProject(
    ...
    try {
      ...
      const projects = response.repository.issues.nodes
        .flatMap((issue: any) => issue.projectItems.nodes.map((item: any) => item.project))
