...
export class GitHubClient {
   ...
   */
  async addIssueToProject(issueNumber: number, projectId: string, owner: string, repo: string): Promise<void> {
    try {
   ...
   */
  async discoverProjects(owner: string, repo: string): Promise<{
    id: string;
    title: string;
    statusFieldId?: string;
    ...
    try {
      ...
      return Array.from(projects.values()).map((project: any) => {
        ...
        return {
          ...
          statusOptions: Object.keys(statusOptions).length > 0 ? statusOptions : undefined,
        };
      });
    } catch (error) {
