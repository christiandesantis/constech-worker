...
class CleanupManager {
  ...
  async executeCleanup(): Promise<void> {
    ...
    logger.debug(`Executing ${this.cleanupFunctions.length} cleanup functions...`);
    ...
    const cleanupPromises = this.cleanupFunctions.map(async (cleanup, index) => {
      try {
        ...
        logger.warning(`Cleanup function ${index + 1} failed:`, error.message);
      }
    });
...
export async function exitGracefully(code: number = 0, message?: string): Promise<never> {
  if (message) {
    if (code === 0) {
