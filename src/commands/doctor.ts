...
export async function doctorCommand(options: DoctorOptions = {}): Promise<void> {
  ...
  for (const check of checks) {
    ...
    if (options.verbose && check.status !== 'pass') {
      console.log(`   ${chalk.gray('└─ Details: ' + getDetailedMessage(check))}`);
    }
...
async function checkBotToken(): Promise<HealthCheck> {
  ...
  try {
    const username = execSync(`GITHUB_TOKEN="${token}" gh api user --jq .login`, { 
      encoding: 'utf-8' 
    }).trim();
...
async function checkEnvironmentVariables(): Promise<HealthCheck> {
  ...
  const missing = required.filter(env => !process.env[env] && !process.env[env.replace('GITHUB_BOT_', 'BOT_APP_')]);
  const foundOptional = optional.filter(env => process.env[env]);
  
  if (missing.length === 0) {
    const message = foundOptional.length > 0 
