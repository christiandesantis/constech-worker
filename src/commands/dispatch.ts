...
export async function dispatchCommand(
	...
	if (options.issue && options.createIssue) {
		await exitGracefully(
			1,
			"Cannot use --create-issue with --issue (issue already exists)",
		);
	}
	...
	if (!botToken) {
		await exitGracefully(
			1,
			`Environment variable ${config?.bot.tokenEnvVar} is required`,
		);
	}
	...
	try {
		...
		// biome-ignore lint/style/noNonNullAssertion: config is validated above
		const executor = new WorkflowExecutor(config!, {
			botToken: botToken ?? "",
			reviewer:
				options.reviewer || process.env[config?.workflow.reviewerEnvVar ?? ""],
...
async function showDryRun(
	...
	console.log(
		`  Project: ${chalk.cyan(`${config.project.owner}/${config.project.name}`)}`,
	);
	console.log(
		`  Base branch: ${chalk.yellow(options.base || config.project.workingBranch)}`,
	);
	console.log(
		`  Bot token: ${chalk.green(config.bot.tokenEnvVar)} ${process.env[config.bot.tokenEnvVar] ? "✓" : "✗"}`,
	);

	const reviewer =
		options.reviewer ||
		process.env[config.workflow.reviewerEnvVar] ||
		config.workflow.defaultReviewer;
	if (reviewer) {
	...
	if (options.createIssue && options.prompt) {
		...
		console.log(
			`     Title: ${chalk.gray("feat: [first sentence of prompt]")}`,
		);
	...
	if (options.issue) {
		console.log(
			`  ${options.createIssue ? "2" : "1"}. ${chalk.blue("Fetch issue")} #${options.issue}`,
		);
	}

	const stepOffset = (options.createIssue ? 1 : 0) + (options.issue ? 1 : 0);
	console.log(
		`  ${stepOffset + 1}. ${chalk.blue("Create feature branch")} from ${options.base || config.project.workingBranch}`,
	);
	console.log(
		`  ${stepOffset + 2}. ${chalk.blue("Execute Claude Code")} in isolated container`,
	);
	console.log(
		`  ${stepOffset + 3}. ${chalk.blue("Run quality checks")}: ${config.workflow.qualityChecks.join(", ")}`,
	);
	console.log(
		`  ${stepOffset + 4}. ${chalk.blue("Create pull request")} with bot authentication`,
	);
	console.log(
		`  ${stepOffset + 5}. ${chalk.blue("Update project status")} to "In Review"`,
	);

	console.log(`\n${chalk.bold("Docker execution:")}`);
	console.log(
		`  Container: ${chalk.gray("Isolated dev container with Claude Code")}`,
	);
	console.log(
		`  Volumes: ${chalk.gray("Project source + Claude Code authentication")}`,
	);
