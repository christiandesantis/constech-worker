...
export async function containersCommand(options: ContainerOptions = {}): Promise<void> {
...
async function listConsWorkerContainers(docker: Docker, showAll: boolean = false): Promise<void> {
  ...
  try {
    ...
    workerContainers.forEach(container => {
      ...
      const statusColor = status === 'running' ? chalk.green : 
                         status === 'exited' ? chalk.yellow : chalk.red;
    ...
    const stoppedContainers = workerContainers.filter(c => c.State === 'exited');
    if (stoppedContainers.length > 0) {
      console.log(chalk.yellow('💡 Cleanup suggestions:'));
      console.log(`   • Remove stopped containers: ${chalk.cyan('constech-worker containers --clean')}`);
      console.log(`   • Force remove all (including running): ${chalk.cyan('constech-worker containers --clean --force')}`);
    }
...
async function cleanupOrphanedContainers(docker: Docker, force: boolean): Promise<void> {
  ...
  try {
    ...
    for (const containerInfo of workerContainers) {
      ...
      if (isRunning && !force) {
        console.log(`  ${chalk.yellow('⚠️  Skipping running container (use --force to remove)')}`);
        continue;
      }
