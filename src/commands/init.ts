...
export async function initCommand(options: InitOptions = {}): Promise<void> {
	...
	try {
		...
		if (!botToken) {
			...
			logger.info(
				"ℹ️  You can set this later or run again with the token configured",
			);
		}
		...
		console.log(
			`📁 Project: ${chalk.cyan(`${detected.owner}/${detected.name}`)}`,
		);
		console.log(
			`🌿 Branches: ${chalk.yellow(detected.defaultBranch)} → ${chalk.green(detected.workingBranch || "staging")}`,
		);
		...
		console.log(
			`1. Review configuration: ${chalk.cyan("constech-worker configure --list")}`,
		);

		if (!botToken) {
			console.log(
				`2. Set bot token: ${chalk.yellow("export GITHUB_BOT_TOKEN=ghp_...")}`,
			);
		}

		console.log(
			`3. Check system health: ${chalk.cyan("constech-worker doctor")}`,
		);
		console.log(
			`4. Start working: ${chalk.cyan("constech-worker dispatch --issue 42")}`,
		);
	} catch (error: any) {
